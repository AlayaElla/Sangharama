...
public class RecipeUI : MonoBehaviour{
	...
	void Start () {
        ...
        recipeSlotsList = recipeUI.transform.FindChild("SlotsList").GetComponent<RectTransform>();
    ...
    void OpenRecipe(GameObject go)
    {
        ...
        btn_Close.transform.FindChild("Text").GetComponent<Text>().text = "返回";
        btn_startRecipe.transform.FindChild("Text").GetComponent<Text>().text = "合成";
    ...
    void CloseRecipe(GameObject go)
    {
        ...
        btn_Close.transform.FindChild("Text").GetComponent<Text>().text = "关闭";
        EventTriggerListener.Get(btn_Close.gameObject).onClick = CloseRecipeMenu;
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            if (map.Slots[i].SlotType == Recipe.SlotTypeList.Material)
            {
                ...
                if (map.Slots[i].MatType == 0)  //Item
                {
                    ...
                    recipeSlot.transform.FindChild("Text").GetComponent<Text>().text = "[" + _item.Name + "]";
                    recipeSlot.transform.FindChild("Bottom/Image").GetComponent<Image>().sprite = Materiral.GetIconByName(_item.IMG);
                    recipeSlot.transform.FindChild("Bottom/Image").GetComponent<Image>().color = new Color(1, 1, 1, 0.5f);
                }
                else if (map.Slots[i].MatType == 1)  //Mind
                {
                    Materiral.Minds _mind = Materiral.FindMindByID(map.Slots[i].MatId);
                    recipeSlot.name = i.ToString();
                    recipeSlot.transform.FindChild("Text").GetComponent<Text>().text = "[" + _mind.Name + "]";
                    recipeSlot.transform.FindChild("Bottom/Image").GetComponent<Image>().sprite = Materiral.GetIconByName(_mind.IMG);
                    recipeSlot.transform.FindChild("Bottom/Image").GetComponent<Image>().color = new Color(1, 1, 1, 0.5f);
                }
            ...
            else if (map.Slots[i].SlotType == Recipe.SlotTypeList.MaterialType)
            {
                ...
                recipeSlot.transform.FindChild("Text").GetComponent<Text>().text = "<color=red>[" + type.Name + "]\n(类型)</color>";
                recipeSlot.transform.FindChild("Bottom/Image").GetComponent<Image>().sprite = Materiral.GetIconByName(type.IMG);
                recipeSlot.transform.FindChild("Bottom/Image").GetComponent<Image>().color = new Color(1, 1, 1, 0.5f);
            }
            else
            ...
            //设置点击事件
            GameObject _slot = recipeSlot.transform.FindChild("Bottom/Image").gameObject;
    ...
    void ClickInBag(GameObject go,object parameter)
    {
        ...
        if (SlotList.ContainsKey(p.ID))
        {
            ...
            slotbox.button.transform.FindChild("Text").GetComponent<Text>().text = goods.Name;
            slotbox.button.transform.FindChild("Bottom/Image").GetComponent<Image>().sprite = Materiral.GetMaterialIcon(goods.MateriralType, goods.ID);
            slotbox.button.transform.FindChild("Bottom/Image").GetComponent<Image>().color = new Color(1, 1, 1, 1f);
    ...
    void SetPropertyPlane()
    {
        ...
        QualitySkillPoint1.transform.FindChild("Handle Slide Area/Handle/Text").GetComponent<Text>().text = recipe.EffetBox.Eft1.NeedQuality.ToString();
        QualitySkillPoint2.value = (float)recipe.EffetBox.Eft2.NeedQuality / 100;
        QualitySkillPoint2.transform.FindChild("Handle Slide Area/Handle/Text").GetComponent<Text>().text = recipe.EffetBox.Eft2.NeedQuality.ToString();
        QualitySkillPoint3.value = (float)recipe.EffetBox.Eft3.NeedQuality / 100;
        QualitySkillPoint3.transform.FindChild("Handle Slide Area/Handle/Text").GetComponent<Text>().text = recipe.EffetBox.Eft3.NeedQuality.ToString();
        ...
        foreach (SlotBox slot in SlotList.Values)
        {
            ...
            parameter.obj = slot;
            GameObject _slotButton = slot.button.transform.FindChild("Bottom/Image").gameObject;
    ...
    void InputMaterial(GameObject go, object parameter)
    {
        ...
        GameObject _slotButton = _slot.button.transform.FindChild("Bottom/Image").gameObject;

        //合成槽效果
        _slotButton.GetComponent<Image>().color = Color.gray;
        GameObject count = _slot.button.transform.FindChild("Count").gameObject;
        count.SetActive(true);
        count.transform.GetChild(0).GetComponent<Text>().text = InputBox.Count.ToString();

        //飞向属性栏
        GameObject _btn = Instantiate(btn_recipeSlot);
        _btn.transform.FindChild("Text").GetComponent<Text>().text = _slot.slot.Name;
        _btn.transform.FindChild("Bottom/Image").GetComponent<Image>().sprite = Materiral.GetMaterialIcon(_slot.slot.MateriralType, _slot.slot.ID);
    ...
    void StartCompose(GameObject go)
    {
        for (int i = 0; i < recipeSlotsList.transform.childCount; i++)
        {
            if (recipeSlotsList.transform.GetChild(i).gameObject.name == "Compose")
                continue;
            GameObject slot = recipeSlotsList.transform.GetChild(i).gameObject;
            slot.transform.FindChild("Bottom/Image").GetComponent<Image>().color = Color.white;
            slot.transform.FindChild("Bottom").GetComponent<Image>().enabled = false;
            slot.transform.FindChild("Text").GetComponent<Text>().text = "";
            slot.transform.FindChild("Count").gameObject.SetActive(false);
        }
            ...
            () =>
            {
                LeanTween.scale(recipeSlotsList.gameObject, new Vector3(0.1f, 0.1f, 0.1f), actionTime * 0.3f).setEase(LeanTweenType.easeInBack).setOnComplete(
                    () =>
                    {
                        ...
                        btn_startRecipe.gameObject.SetActive(true);
                        btn_startRecipe.transform.FindChild("Text").GetComponent<Text>().text = "收取";
                        ...
                        targetgoods.transform.FindChild("Text").GetComponent<Text>().text = goods.Name;
                        targetgoods.transform.FindChild("Bottom/Image").GetComponent<Image>().sprite = Materiral.GetMaterialIcon(goods.MateriralType, goods.ID);

                        GameObject _targetButton = targetgoods.transform.FindChild("Bottom/Image").gameObject;
    ...
    void AddPropertyUIEft(CharBag.Goods _slot)
    {
        ...
        for (int i = 0; i < _slot.Property.Length; i++)
        {
            ...
            Color _color_text = _property.transform.FindChild("Text").GetComponent<Text>().color;
            Color from = new Color(_color_image.r, _color_image.g, _color_image.b, 0);
            _property.GetComponent<Image>().color = from;
            _property.transform.FindChild("Image").GetComponent<Image>().color = from;
            _property.transform.FindChild("Text").GetComponent<Text>().color = from;
            ...
            LeanTween.value(_property.gameObject, from, _color_text, 0.25f).setDelay(actionTime).setOnUpdate(
                (Color col) =>
                {
                    _property.transform.FindChild("Text").GetComponent<Text>().color = col;
                }
                );
            LeanTween.value(_property.gameObject, from, new Color(1,1,1,1), 0.25f).setDelay(actionTime).setOnUpdate(
                (Color col) =>
                {
                    _property.transform.FindChild("Image").GetComponent<Image>().color = col;
                }
                );
            ...
            _property.transform.FindChild("Text").GetComponent<Text>().text = _p.Name;
            _property.transform.FindChild("Image").GetComponent<Image>().sprite = Materiral.GetIconByName(_p.IMG);
